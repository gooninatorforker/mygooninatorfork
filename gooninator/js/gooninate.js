...
/***/ function(module, exports) {
	...
	*/
	...
	var fetch = function fetch(tumblr, offset) {
	  ...
	  var url = 'https://api.tumblr.com/v2/blog/' + tumblr + '.tumblr.com/posts?' + 'offset=' + offset + (tumblr == "iwantmygflikethis" ? '&tag=caption' : '') + (tumblr == "dorisconquered" ? '&tag=interracial' : '') + (tumblr == "overwhelmed-by-eroticism" ? '&tag=cleanup' : '') + (tumblr == "allcuckold" ? '&tag=Cuckold+cleanup' : '')  + (tumblr == "specific-filth" ? '&tag=public' : '') + (tumblr == "cum-in-kleenex" ? '&tag=creampie' : '') + (tumblr == "sissycucksexual" ? '&tag=cuckold' : '') + (tumblr == "whitesarethepast" ? '&tag=bbc' : '') + (tumblr == "confirmedloser" ? '&tag=humiliation' : '') + (tumblr == "cuckoldcaps" ? '&tag=cum+eating' : '') + (tumblr == "glamcuck" ? '&tag=caption' : '') + (tumblr == "fuckyeahfriendlyfire" ? '&tag=creampie+fucking' : '') + (tumblr == "pornocuck" ? '&tag=cuckold' : '') + '&limit=20&' + 'api_key=' + API_KEY;
	  $.ajax({
	    method: 'GET',
	    cache: false,
	    dataType: 'jsonp',
